...
type crypto$Sign$private_key = string | { key: string, passphrase: string, ... }
...
declare module 'fs' {
  ...
  declare function mkdir(
    path: string,
    mode?: number | { recursive?: boolean, mode?: number, ... },
    callback?: (err: ?ErrnoError) => void
  ): void
...
declare module 'url' {
  ...
  declare type UrlWithStringQuery = {| ...Url, query: string | null |}
...
type process$CPUUsage = { user: number, system: number, ... }
...
declare class Process extends events$EventEmitter {
  ...
  uptime(): number;
  version: string;
  versions: { [key: string]: ?string, node: string, v8: string, ... };
}
